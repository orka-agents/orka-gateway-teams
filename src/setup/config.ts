...
export interface SetupConfig {
  appId: string; tenantId: string; clientSecret: string; host: string; port: number;
...
export function parseSetupConfig(env: NodeJS.ProcessEnv): Readonly<SetupConfig> {
  try {
    ...
    return validateSetupConfig({ appId: guid(env.TEAMS_APP_ID), tenantId: guid(env.TEAMS_TENANT_ID),
      clientSecret: secret(env.TEAMS_CLIENT_SECRET), challengeFile: path(env.SETUP_CHALLENGE_FILE), captureFile: path(env.SETUP_CAPTURE_FILE),
...
// Kept local because ingress credential parsers are private; normal receiver policy is unchanged.
